<?php
  ...
  $programa="SELECT * FROM insumos INNER JOIN proveedores ON insumos.id_proveedores=proveedores.id_proveedores ORDER BY nombre";
                  ...
                  <div class="form-group">
                    <div class="col-sm-4">
                      <label for="" class="control-label">Insumo</label>
                        <?php
                          echo "<select  name='insumo' id='insumo' required class='form-control' onchange='cargarAjax()'>
                          <option value=''>Seleccione</option>";
                          while($row=mysqli_fetch_assoc($resultado)){
                          echo "<option value='".$row['id_insumos']."'>".$row['nombre']." - ".$row['marca']."</option>";
                          }
                          echo "</select>";
                        ?>
