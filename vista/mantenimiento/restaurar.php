...
define("BACKUP_FILE", 'cdaps.sql.gz'); // Script will autodetect if backup file is gzipped based on .gz extension
define("CHARSET", 'utf8');
...
class Restore_Database {
                            ...
                            if (preg_match('/;$/', $line)) {
                                ...
                                if(mysqli_query($this->conn, $sql)) {
                                    if (preg_match('/^CREATE TABLE `([^`]+)`/i', $sql, $tableName)) {
                                        $this->obfPrint("Tabla correctamente creada: `" . $tableName[1] . "`");
                                    }
                                    $sql = '';
                                } else {
    ...
    protected function gunzipBackupFile() {
        ...
        $this->obfPrint('Archivo de copia de seguridad ' . $source . '... ', 0, 0);
...
$result = $restoreDatabase->restoreDb(BACKUP_DIR, BACKUP_FILE) ? 'Exitoso' : 'No se encontro respaldo de la BD';
